...
- **spec-ai-writer の想定利用範囲**: `spec-ai-writer` はローカルホスト（同一マシン）での実行を前提とした開発支援ツールです。API に認証機構はないため、ネットワークに公開しないでください（[spec-ai-writer/README.md](spec-ai-writer/README.md#セキュリティ) 参照）
