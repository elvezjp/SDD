...
このリポジトリは、仕様駆動開発の練習用サンプルとして提供されています。他の読者の参考用として維持するため、以下の設定を行っています。

- **Branch Protection Rules**: `main`ブランチは保護されており、直接の編集（プッシュ）はできません
- **オーナーの編集**: リポジトリのオーナーは、必要に応じて編集・更新を行います
- **読者の利用方法**: 編集したい場合は、必ずフォークまたはクローンしてから作業してください
...
- 画面右上の「Fork」ボタンをクリック
- 自分のアカウントにコピーが作成されます
- フォークしたリポジトリは自由に編集・プッシュできます

**方法B：クローンする**
- ローカルにコピーして編集することもできます
- ただし、元のリポジトリは保護されているため、変更をプッシュすることはできません
- ローカルでのみ編集・実験できます

### リポジトリの目的

このリポジトリは、『仕様駆動開発 実践入門』（仮題）の第1章「30分で体験する仕様駆動開発」で使用する練習用リポジトリです。

- 仕様駆動開発の4つの原則と7つの工程に沿ったサンプルファイルが含まれています
- リポジトリ構造の参考例として活用してください
- 第1章のステップ0.5で、このリポジトリの確認方法やフォーク・クローンの手順を詳しく説明しています

### 更新方針

- リポジトリのオーナーが、必要に応じて内容を更新します
- 誤字脱字の修正、サンプルファイルの改善などは、随時反映されます
- 大きな変更がある場合は、コミットメッセージで変更内容を明記します
